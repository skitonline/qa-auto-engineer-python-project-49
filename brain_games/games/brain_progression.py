...
def play_brain_progression():
    ...
    how_many_generate = [randint(STEP_TO, GENERATE_TO) 
    ...
    questions = [[count_number(start[j], step[j], i) 
                  for i in range(how_many_generate[j])] 
                  for j in range(NUMBER_OF_ROUNDS)]
    
    hide_index = [randint(0, how_many_generate[j] - 1) 
                  for j in range(NUMBER_OF_ROUNDS)]
